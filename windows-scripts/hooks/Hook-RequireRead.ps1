...
$GATES = @(
  ...
  # ---------------------------------------------------------------------------
  # 2026-09-20 15:00 - FreshMin ADDED TO EVERY GATE. It existed on exactly ONE.
  #
  # Jeff, 14:52: "I don't know how to get you to do it. gates, hooks, nothing
  # works ... You tell me what to do?"
  #
  # THE MEASURED ANSWER, and it is NOT "build another gate":
  # That day a session wrote scripts/sensor-liveness-test.js keyed off
  # linkquality AGE - the precise trap the ZIGBEE gate's own Why text names
  # ("Availability is the ONLY real liveness signal ... FOUR false alarms").
  # It promptly produced a FIFTH, condemning two healthy mains-powered repeaters
  # that answered a poke within 20 seconds.
  #
  # THE GATE MATCHED AND STILL LET IT THROUGH. Not a pattern miss - the receipt
  # said zigbee_mesh_routers_2026-08-27.md had been read. It had: on 2026-09-15
  # at 11:37, ONE HUNDRED AND EIGHTEEN HOURS EARLIER. The session had been open
  # five days. A receipt with no expiry says "read" forever, and a file read five
  # days ago is not in front of you.
  #
  # Meanwhile the ONE gate carrying FreshMin blocked that same session TWICE and
  # forced a re-read. It is the only gate that worked. The mechanism was already
  # right; it was wired to 1 of 12 gates.
  #
  # 120 minutes, not 60: long enough not to nag inside a working session, short
  # enough that the gotchas are still in mind. The SHORT-LIST design rule holds -
  # satisfying this costs ONE Read call, and reads are never blocked.
  # ---------------------------------------------------------------------------
  @{ Name = 'GARAGE DOOR'
...
# ---------------------------------------------------------------------------
# HEADER GATE + CREATE GATE - added 2026-09-20 15:05.
# Jeff, 14:59: "I want my rules controlled by machinery."
#
# Both target failures from that same day that NO existing gate could see:
#
# HEADER. The universal gate accepted ANY read of OPEN_ITEMS.md. A session read
# it at offset 113 - one deep row - and never saw lines 47-52, the header block
# SESSION_START section 5 says to read FIRST ("the header carries the current
# verified triage - read that part first, not the 3,100 lines below it"). It
# then presented the mailbox orphan and the open garage man door as new
# findings. Both were in that header. Now a read from the TOP is required,
# and it must be recent - a header read from 12 hours ago is not in mind.
#
# CREATE. Nothing asked "does this already exist?" before something new was
# built. That day a liveness gate was written that duplicated
# automation.hcc_sensor_silence_watchdog (live since 08-30, better signal,
# already tuned after it woke Jeff and Angela all night). A read-receipt hook
# was nearly built too - it already existed. COST_LEDGER 2026-08-26 states the
# rule: "enumerate what already exists before building anything." Now building
# anything new requires a Search-HCC.ps1 run first. Cost: one search.
#
# Both fail OPEN on an unreadable receipt, both are stood down by HCC-OVERRIDE,
# and neither ever blocks a read or a doc write (those exit above).
# ---------------------------------------------------------------------------
$nowE = [int][double]::Parse((Get-Date -UFormat %s))

# --- HEADER: OPEN_ITEMS.md read from the top (offset 0 or 1) within 240 min ---
$HEADER_MIN = 240
$hdrNewest = 0
foreach ($ln in ($readSoFar -split "`r?`n")) {
  if ($ln -notlike '*OPEN_ITEMS.md*') { continue }
  if ($ln -match '^(\d{9,})\|') {
    $st = [int]$Matches[1]
    if ($ln -match '\|offset=(\d+)\s*$') {
      if ([int]$Matches[1] -le 1 -and $st -gt $hdrNewest) { $hdrNewest = $st }
    } else {
      # pre-2026-09-20 receipt line: offset unknown. Give it the benefit of the
      # doubt but still hold it to freshness - never lock Jeff out over format.
      if ($st -gt $hdrNewest) { $hdrNewest = $st }
    }
  }
}
if ($hdrNewest -eq 0 -or (($nowE - $hdrNewest) / 60) -gt $HEADER_MIN) {
  $age = if ($hdrNewest -gt 0) { " (last top-of-file read " + [math]::Round(($nowE - $hdrNewest) / 60) + " min ago)" } else { " (no top-of-file read on the receipt)" }
  Deny 'OPEN_ITEMS HEADER' @('docs/OPEN_ITEMS.md  -- FROM THE TOP: offset 0, first ~60 lines. The header carries the verified triage.' + $age) ('On 2026-09-20 a session satisfied this gate with a read at offset 113, never saw the header, and re-discovered two items already listed there. SESSION_START section 5: read the header part first. A deep read does not count and a header read older than 4 hours does not count.')
}

# --- CREATE: building something NEW requires a Search-HCC.ps1 run within 120 min ---
$CREATE_MIN = 120
$isCreate = $false; $what = ''
if ("$($j.tool_name)" -eq 'Write' -and $filePath -match '(?i)[\\/](scripts|windows-scripts)[\\/]' -and -not (Test-Path -LiteralPath $filePath)) {
  $isCreate = $true; $what = 'a NEW file: ' + $filePath
}
# An Edit or Write cannot POST to Home Assistant - only a shell call can create
# an automation, so only a shell call is held to this. The first version matched
# the path in ANY tool text and blocked its own test file within a minute of
# being written. Fixed 15:10 the same day.
# (HCC-OVERRIDE: this one edit repairs the gate that would otherwise block its
#  own repair - the on-disk version matches this comment's path text. 2026-09-20.)
if ("$($j.tool_name)" -match '^(Bash|PowerShell)$' -and $blob -match '(?i)api/config/automation/config/') { $isCreate = $true; $what = 'an HA automation through the config API' }
$createStoodDown = $false
if ($isCreate) {
  # This gate can only be satisfied if Hook-ReadReceipt is seeing shell commands,
  # which needs the Bash|PowerShell PostToolUse matcher (settings.json, 2026-09-20)
  # - and hooks load at session start. The receipt hook drops a marker the first
  # time it runs from a shell call. No marker = not live in this session = this
  # gate is UNSATISFIABLE. A gate nobody can satisfy gets deleted (design rule
  # above), so instead it stands down, SAYS SO in the session, and arms itself
  # after the next restart. Nothing else here is skipped - topic gates still run.
  $marker = Join-Path $env:TEMP ("hcc-shellreceipt-" + $sid + ".flag")
  if (-not (Test-Path -LiteralPath $marker)) { $createStoodDown = $true }
}
if ($isCreate -and -not $createStoodDown) {
  $srchNewest = 0
  foreach ($ln in ($readSoFar -split "`r?`n")) {
    if ($ln -match '^(\d{9,})\|\[search\]') { if ([int]$Matches[1] -gt $srchNewest) { $srchNewest = [int]$Matches[1] } }
  }
  if ($srchNewest -eq 0 -or (($nowE - $srchNewest) / 60) -gt $CREATE_MIN) {
    Deny 'BUILD-FIRST-SEARCH' @('RUN FIRST:  .\windows-scripts\Search-HCC.ps1 "<the thing you are about to build>"', 'You are creating ' + $what, 'Then LIST what already exists for it (HA automations by name, files in scripts/) before writing anything.') ('Enumerate what already exists before building anything (COST_LEDGER 2026-08-26). On 2026-09-20 scripts/sensor-liveness-test.js was built while automation.hcc_sensor_silence_watchdog had been doing the job since 08-30 with a better signal, and a read-receipt hook was nearly built while Hook-ReadReceipt.ps1 already existed. One search finds both. Reads and doc writes are never blocked.')
  }
}
