...
markdown that would be republished forever for zero benefit — the search tool reads iCloud, not
...
|---|---|---|
| **iCloud** — `HCC-Archive\MASTER-RECORD\` | 219 files, 126 MB: 6,896 verbatim messages, 81 decisions in Jeff's words, all 635 commits, 187 images, the full 05-20→08-16 chronicle | **Searching.** This is what the tools read. |
| **Git** — branch `claude/task-completion-4a4wmo`, commit `f036f84` | `history-archive/` — the 23 reconstructed history files, verified **byte-identical** to the iCloud copy | Durability. Recoverable forever, even if iCloud is lost. |
...
Both are capped for cost — a search is ~2.5k tokens, a status briefing ~600. Neither is ever
auto-loaded.
