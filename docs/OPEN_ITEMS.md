...
| 105 | 🔴 **ELECTRIC Today/Yesterday/Peak Hour/Last 7 Days are POLLING ARTIFACTS, not usage — found 2026-08-31 by finally running the live WS check the 08-06 doc said was still owed.** SmartHub syncs ONCE A DAY at 08:00 and dumps the whole previous day into a single hourly bucket: measured 08-30 08:00 = 51.0 kWh, 08-31 08:00 = 55.0 kWh, **all other 30 hours = 0.0** (2 of 32 hours carry any movement). So **Peak Hour will read 8 AM every day forever** and Today reads 0 until 08:00 then jumps a full day. The 08-06 premise — *"the integration aligns the time of use with the time the usage actually happened"* — is FALSE on this instance. **This Month + Est. Cost are UNAFFECTED and correct** (monthly total real: state 1622→1777). Both 08-06 bug fixes ARE confirmed working live (`recorder/statistics_during_period` accepted; `change` populated in only 3 of 57 buckets, so the `toDiffedSeries` fallback is right). **Do NOT rebuild without Jeff's say-so.** Options: drop the four sub-daily cells as un-sourceable, or relabel them daily. Real hourly needs the CT-clamp build — and per the 08-06 rule, do NOT fake 'Now'. | me | 0d | |
