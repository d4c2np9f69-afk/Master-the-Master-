...
| 86 | 🟠 **MAILBOX LINK IS 0–3 AND HAS BEEN SINCE 08-27 19:00 — measured, and it contradicts the mesh doc.** `sensor.0xa4c138f1b3e16c9e_linkquality` read **0** at 08-27 19:00 and has since logged only **3 / 0 / 3 / 0** (11:50, 15:55, 17:07 on 08-28). Z2M's own overview counts it as the **1 device under LQI 50**. Only **13 messages in 3.7 days**, worst gap **8.05 h** — the sparsest device in the house by a wide margin. | **JEFF / CLAUDE** | opened 08-28 | ⚠️ **`docs/zigbee/zigbee_mesh_routers_2026-08-27.md` records "Mailbox 0 → 76" after the 08-27 re-pair. It is not 76 now.** Either it drifted straight back or the 76 belonged to something else — **`Floating Repeater` currently reads exactly 76**, so a mix-up is possible. **Do not correct that doc from this note alone; re-measure first.** It still reports, so it is not dead — but it is the device most likely to trip #84 if the timeout is tightened. |
