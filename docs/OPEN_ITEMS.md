...
| 55 | 🟠 **Apple TV SMB "empty file" root-caused.** All 12 failed logons were from 192.168.1.104: `0xC000006A` wrong password ×4, `0xC0000072` account disabled ×3, then `0x80090308 SEC_E_INVALID_TOKEN` ×2 with an EMPTY account name. Reproduced on the box: blank-password Guest → **System error 86**; `tv`/`movies` → **success, 380 entries**. Cause = **`LimitBlankPasswordUse=1`**, which was never in APPLETV_SHARE.md's change list — so the "no password" path was recorded as proven on 08-23 without ever being exercised the way VLC uses it. | **JEFF** | 08-23 | Set to **0** with Jeff's authorization. **Microsoft documents that a reboot is required**, and that was confirmed empirically (still failed with the value at 0). Deliberately did NOT reboot — the Beast had just returned from the outage and a reboot takes CodeProject.AI and the whole house AI dark. **Use `tv`/`movies` until the next natural reboot, then verify with the boxes empty.** Windows 11 **Home** has no `secpol.msc`, so the registry is the only route on this machine. Detail: `HCC-secrets/APPLETV_SHARE.md`. |
...
| 58 | 🟠 **Two HomeKit bridges appear UNPAIRED since the 2026-08-21 restart, and nobody noticed.** Two `HomeKit Pairing` notifications have sat unread since **2026-08-21T22:40:00Z** — the exact restart that also started the 44 h Zigbee blackout (#44). **GLE 350 Lock:21065** → code `213-62-736`; **HCC Home (HASS Bridge)** → code `052-96-029`. HA only posts a pairing prompt for a bridge with no paired controller. Tellingly, **HCC Cameras:21081 has NO such notification and demonstrably works** (feature test PASS + Jeff confirmed the popup 08-23), which is exactly the pattern you would expect if those two lost pairing and Cameras did not. | **JEFF** (needs his iPhone) | found 08-23 | If unpaired, Apple Home / Siri / Watch / CarPlay have had **no** access since Friday to: `lock.gle_350_lock`, `light.livingroom_cans`, `input_boolean.night_mode`, the six irrigation switches, `scene.turn_on_sharky`, the HCC scripts, `alarm_control_panel.blink_loewen301`, backyard temp/humidity. **All 3 bridges report `state=loaded` and NO integration is in a failed state** — `loaded` means the integration started, NOT that a controller is paired, which is exactly why this stayed invisible. ⚠️ **CAMERA FREEZE: not touched.** Re-pairing is Jeff's hands in the Home app. Verify against `docs/beehive/homekit_tracker.md` before any change. |
| 59 | 🟡 **"Camera AI is DOWN" fired again 2026-08-23 13:30 CT — false, as #47 predicted.** CodeProject.AI is UP (service Running, port 32168, restarted 15:40:13 after the outage) and the camera feature test PASSED end-to-end twice at 16:24/16:27. The alarm triggers on "no `object_detected` in 6+ h while motion fires," which a genuinely quiet house also produces. **This is now the second recorded false fire.** | CLAUDE | #47 reopened 08-23 | Needs a test that distinguishes "AI host unreachable" from "house was quiet" — e.g. ping `192.168.1.194:32168/v1/status/ping` rather than inferring from event absence. Not built. Left alone pending Jeff, since it touches the camera stack. |
| 60 | 🟡 **Recurring `Login attempt failed` from localhost.** `Login attempt or request with invalid authentication from localhost (127.0.0.1). Requested URL: '/api/camera_proxy/camera.301_front_doorbell?token=...'` — 8 occurrences logged 08-23 08:00, plus a standing persistent notification from 13:00. A camera_proxy signed-token URL is being retried after its token expired. | CLAUDE | found 08-23 | Cosmetic so far — no functional impact observed, doorbell frame path is documented as upstream-limited anyway. Logged rather than handed off in prose. Do not chase inside the camera freeze without Jeff. |
