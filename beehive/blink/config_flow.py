"""Config flow to configure Blink with OAuth 2FA fix for HA Core 2026.6.x."""
...
async def _send_blink_2fa_pin(blink: Blink, pin: str | None) -> bool:
    ...
    Home Assistant Core 2026.6.x still calls Blink.send_2fa_code(), but
    blinkpy 0.25.x OAuth stores the temporary state on Auth and completes via
    Auth.complete_2fa_login().  This keeps the same HA flow but uses the
    current blinkpy OAuth path first.
    """
    if pin is None:
        pin = ""

    auth = getattr(blink, "auth", None)
    complete_2fa = getattr(auth, "complete_2fa_login", None)
    if callable(complete_2fa):
        success = await complete_2fa(pin)
        if not success:
            raise BlinkSetupError
        try:
            blink.setup_urls()
            await blink.get_homescreen()
            if not blink.last_refresh:
                import time
                blink.last_refresh = int(time.time() - blink.refresh_rate * 1.05)
            success = await blink.setup_post_verify()
...
class BlinkConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_2fa(
        ...
        return self.async_show_form(
            step_id="2fa",
            data_schema=vol.Schema({vol.Required(CONF_PIN): vol.All(str, vol.Length(min=1))}),
            errors=errors,
            description_placeholders={"note": "Enter the SMS code Blink just sent."},
        )
