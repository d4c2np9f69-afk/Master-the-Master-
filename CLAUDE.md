...
11. **Leak-detection automation, zero new hardware.** Two complementary approaches discussed 08-01: (a) simple idle-flow logic automation (alert if flow is running while the house is idle/asleep and nothing known is using water) — fast, catches sudden bursts; (b) the free HACS `Water-Monitor` integration (github.com/markaggar/Water-Monitor) — its low-flow "dribble" and toilet-refill-pattern detectors. **Important calibration note (confirmed 08-01, not the "meter is broken" theory from earlier the same day):** this meter's real update cadence is batched/threshold-based — observed gaps from ~20 minutes to ~3 hours between actual value changes, not a steady live stream. Water-Monitor's default timers assume a much more continuous flow-rate feed, so its low-flow detector will likely be significantly slower to trip than out-of-the-box defaults suggest; the idle-flow logic approach (a) is less sensitive to this since it only needs "is flow currently nonzero," not a live rate. No longer blocked — safe to start either whenever Jeff wants.
