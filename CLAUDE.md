...
5. **Lighthouse score** — 60/100, low priority (unminified single-file index.html).
...
8. **Build the Mercedes-GPS backup trigger into Angela's Almost Home (08-01).** `automation.angela_almost_home` still hasn't fired since 07-16 — the 07-26 mitigation isn't enough. Jeff's idea confirmed viable live: `device_tracker.gle_350_device_tracker` flips `home`→`not_home` within ~1-2 minutes of the car actually moving (verified in real time 08-01 as Angela drove off — no manual refresh needed, mbapi2020 pushes updates via its own websocket on a real schedule). Next step: wire this tracker in as a secondary/backup trigger alongside her phone's zone trigger.
9. **Zigbee alarm hardware — Jeff purchasing 07-31.** Bare-bones parts list (coordinator, door sensors, siren) plus the full Guardian safety-layer wishlist (smoke, water leak, freeze, water-main shutoff) researched and handed to Jeff as a downloadable file this session — real current products verified via web search, not guessed. Once hardware arrives and is paired in HA, item 4 (Panic automation) becomes buildable — `docs/beehive/panic_alarm_automation.md` already has the automation YAML ready, just needs real entity ids swapped in.
