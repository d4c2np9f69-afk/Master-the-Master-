...
16. **"Alexa, fast forward the commercials" — REAL ROOT CAUSE FOUND via HA source + Amazon forums (08-03), needs Jeff to redo the Alexa Routine with a different trigger phrase.** Jeff confirmed the intended design: say "Alexa, fast forward" while watching, it skips the commercial via `script.hcc_skip_commercial` (3× ADB `keyevent 90` + resume via `keyevent 126` — confirmed correct keycodes, script itself is built right), then resumes play automatically (already built in). **Two independent, source-verified reasons the literal phrase "fast forward" can never trigger this, no matter how Beehive/ADB is configured:** (1) Read HA core's actual `homeassistant/components/alexa/handlers.py` directly — the Alexa Smart Home integration implements Play/Pause/Stop/Next/Previous directives but has **no handler at all for `PlaybackController.FastForward`/`Rewind`** (matches open HA issue home-assistant/core#87327) — so if `media_player.fire_tv_viewing_room` is exposed to Alexa directly, saying "fast forward" sends a directive HA silently can't handle. (2) Even routing around that via a custom Alexa Routine (the documented pattern in `docs/beehive/ha_helpers_and_alexa.md` Part 6B1), Amazon's own forums confirm **built-in-sounding phrases like "fast forward" get intercepted as native Alexa media commands and never reach the custom Routine at all** (same class of issue as "Alexa, bedtime" reports on Amazon's community forum) — this is an Alexa-side reserved-word behavior, not something HA or this project can configure around. **The fix: Jeff needs to rebuild the Alexa Routine with a trigger phrase that doesn't sound like a native media command** — e.g. "Alexa, skip the commercial" or "Alexa, TV skip" instead of "fast forward" — wired to Smart Home → activate the exposed `script.hcc_skip_commercial` (scripts expose to Alexa as a Scene). This is Alexa-app + HA-expose-settings work only Jeff (Routines are account-level, in the Alexa app) or the coworker (HA expose toggle) can do — nothing in this repo to change, and Routines aren't stored in `beehive-config/` YAML at all. Separately, `media_player.fire_tv_viewing_room`'s live state / ADB pairing health is still worth Jeff/coworker checking (Developer Tools → States) since it affects the PiPup camera automation too, but it is NOT the cause of the fast-forward phrase failing — that's the two reasons above, confirmed independent of ADB health.
