...
9. **📺 Fire TV motion pop-up alerts — IN PROGRESS (started 07-11).** Fire TV Stick paired to HA (`media_player` entity "Fire TV - Viewing Room", see 07-11 change log). `alexa_media_player` HACS integration installed + HA restarted; **Jeff needs to finish the Amazon login (email/password + 2SV authenticator key) in the integration setup wizard on the beast's browser** — Claude does not handle credentials, this was left on-screen for him. **Once that's done, still need to build:** (a) add a "show alerting camera on Fire TV" action to the existing `AI Object Detected Notify` automation in `packages/hcc.yaml` (via `alexa_media_player`'s send-command service, so it fires the same Alexa "Show Camera" behavior but from HA — reuses the existing person/vehicle/animal classification + per-camera mute gate already built for phone notifications); (b) a new arrival-suppression automation — trigger on `person.*` (Angela/Jeff/Braxton) going `not_home`→`home`, action = set the matching `input_datetime.hcc_ai_mute_<camera>` helper ~10 min out, so cameras near the door don't alert on family walking in.
