...
4. **`automation.ai_show_camera_on_fire_tv` needs a real disable, not just runtime `turn_off`.** Its entity_registry entry returns "Entity not found" on both `get`/`update` — a genuine desync. Currently off only because of a runtime `automation.turn_off` call, which will NOT survive the next HA restart (it'll silently re-enable and fight with the new `HCC — AI Camera Popup on Fire TV` automation). Needs its actual YAML block found and disabled/deleted directly in `packages/hcc.yaml`.
...
9. **Angela's Almost Home automation isn't actually working (Jeff, 07-31).** `automation.angela_almost_home` is enabled but hasn't fired since 07-16 — the 07-26 mitigation (distance-based backup trigger) isn't enough on its own. Root cause is still Angela's phone (iOS location permission/Background App Refresh), not fixable from Beehive. **Jeff's idea: use the Mercedes GLE's own GPS as a secondary/backup presence signal** (car entities already live via mbapi2020, see CAR reference) — worth checking if the car's location can serve as a reliable proxy or fallback trigger alongside her phone.
10. **Zigbee alarm hardware — Jeff purchasing 07-31.** Bare-bones parts list (coordinator, door sensors, siren) plus the full Guardian safety-layer wishlist (smoke, water leak, freeze, water-main shutoff) researched and handed to Jeff as a downloadable file this session — real current products verified via web search, not guessed. Once hardware arrives and is paired in HA, item 5 (Panic automation) becomes buildable — `docs/beehive/panic_alarm_automation.md` already has the automation YAML ready, just needs real entity ids swapped in.
