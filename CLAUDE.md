...
- **08-01 (coworker, latest):** **Diagnosed a real, distinct water-meter fault by live-testing with actual water use — not guessing.** After explaining the leak-detection ideas (Water-Monitor HACS integration, idle-flow logic), needed the pit radio's real report cadence, so ran a controlled test: triggered a B-Hyve irrigation zone via HA + Jeff took a real shower, both running simultaneously for ~47 min. Result read straight from the `rtlamr2mqtt` add-on's raw decoder log (`ha addons logs 6713e36e_rtlamr2mqtt`): the water meter (endpoint `79453337`) was freshly decoded on every capture the whole time (heartbeat/`sensor.water_meter_last_seen` updated normally every ~1.5-5 min) but reported the exact same value, `179097`, on every single read — zero movement despite substantial confirmed usage. In the identical window the gas meter (`33393066`, same dongle/antenna/pipeline) ticked normally (`885060→885062`), ruling out the dongle/software/HA entirely. Jeff confirmed irrigation and the house shower share one municipal supply, ruling out "wrong source." **Conclusion: the pit radio's register is stuck — it's still transmitting a heartbeat, but endlessly re-broadcasting a stale reading.** This is the same physical unit that went fully silent 07-28 (fixed, closed 07-31) — recurring, but with a different, more diagnostic symptom this time. Jeff needs to call WHUD again with account `00710690-02` / meter S/N `25394131` / endpoint `79453337`, specifically flagging that the radio is transmitting normally but the register value isn't advancing despite confirmed usage (a more precise report than the last "went silent" call). Both software leak-detection ideas from earlier today are blocked until this hardware fault is fixed — see Pending Items.
...
10. **Water meter pit radio (endpoint `79453337`) needs Jeff to call WHUD again — CONFIRMED via live test 08-01, not guessed.** Radio transmits a normal heartbeat but the register is stuck re-broadcasting the same value (`179097`) regardless of real usage — see 08-01 Change Log entry for the full test (irrigation zone + shower run simultaneously, raw decoder log proof, gas meter on the same dongle ticked normally in the same window). Same physical unit as the 07-28 total-silence fault (closed 07-31) — recurring. Account `00710690-02`, meter S/N `25394131`. Tell WHUD specifically that the transmitter heartbeat is normal but the value isn't advancing — a more precise symptom than last time. **Blocks both leak-detection ideas below (item 11) until fixed.**
11. **Leak-detection automation, zero new hardware — ready to build once item 10 is fixed.** Two complementary approaches discussed 08-01: (a) simple idle-flow logic automation (alert if flow is running while the house is idle/asleep and nothing known is using water) — fast, catches sudden bursts; (b) the free HACS `Water-Monitor` integration (github.com/markaggar/Water-Monitor) — its low-flow "dribble" and toilet-refill-pattern detectors, but temper expectations: the meter's real report cadence is a fresh catch every ~1.5-5 min with the value only changing when enough usage has accumulated, so it'll be slower to trip than the integration's defaults assume. Don't start either until the meter itself is confirmed fixed (item 10) — right now the underlying data feed is unusable for this regardless of approach.
