...
9. **Remaining light-mode contrast gap (08-01) — needs a Jeff decision, not a safe swap.** After the 08-01 token-swap fixes, ~20 elements still fail WCAG contrast in light mode, all tied to the shared `--gold` (3.35-3.5:1, e.g. section header labels) or `--muted` (4.0-4.4:1, e.g. utility card provider text, status chips) tokens themselves being not-quite-dark-enough — not a wrong-color bug like the ones already fixed, the correct token is already in use. Fixing this means either darkening `--gold`/`--muted` globally (ripples across many more elements than just the failing ones) or increasing font-weight/size in specific spots. Real visual design call — ask Jeff before touching it.
