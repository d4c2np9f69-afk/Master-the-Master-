...
**⚠️ STATUS (2026-07): FOUNDATIONAL FIX NEEDED before meters/RTL-SDR/Zigbee/alarm. Current state (confirmed by Jeff): HA was flashed to an EXTERNAL USB drive and boots from that; **Windows 10 is still on the internal eMMC** (never wiped). HA "comes up" and is usable (Jeff's been adding Blink/B-Hyve this way) but **booting HA OS off an external USB drive is the likely cause of the flakiness** (homeassistant.local intermittent → 192.168.1.66 fallback, integrations not sticking). FIX (recommended): (1) in current HA, Settings → System → Backups → create + DOWNLOAD a backup; (2) install **HA OS onto the INTERNAL eMMC** (wipe Win10) so it boots standalone + frees USB ports for RTL-SDR/Zigbee; (3) restore the backup. **CONFIRMED: the Beelink is DEDICATED to Beehive** (Jeff revived it from a drawer to be the brains) → plan = wipe Win10, install HA OS to the internal eMMC. **Step-by-step guide saved at `docs/beehive/HA_OS_setup_J45.md`** (back up → Ubuntu live USB → flash HA OS to internal → restore backup). Get Beehive solid BEFORE buying/adding more.**
