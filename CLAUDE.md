...
- **07-15 (coworker/beast session, later):** 🎯 **Fire TV pop-up now does a REAL universal pause, not a hope-it-backgrounds-cleanly pause — fixes it for every streaming app, not just Fubo.** Jeff's actual ask: when a Blink alert fires, the show should genuinely pause and resume from that exact moment — for whatever app he's in, not just Fubo. Investigated via HA community forums + Amazon's own Fire TV dev docs first (per Jeff's ask), then verified live on the real device via ADB before touching anything. **Root cause of the old behavior:** step 1 of `AI Show Camera on Fire TV` sent `input keyevent 127` (raw MEDIA_PAUSE key event) hoping it would pause whatever was playing — **confirmed via live testing this does nothing** (checked `dumpsys media_session`: Fubo's position kept advancing, state stayed "playing"). It had never been verified working — a repeat of the exact "declared done without testing" pattern the whole debugging protocol exists to prevent. **The real fix, confirmed working live:** `adb shell cmd media_session dispatch pause` — this calls the Android MediaSession API's pause callback directly (system-wide, targets whatever app currently holds the active session) rather than simulating a remote-button press. Confirmed via `dumpsys media_session`: position froze exactly and held steady (verified via two identical-frame screenshots a few seconds apart — genuinely paused, not just self-reporting paused). Resume via `input keyevent 126` (or `dispatch play`) picks back up from that **exact frozen position** for on-demand/DVR content (tested on an actual Fox News DVR recording: paused at 335890ms, resumed at 343307ms — the difference is just the few seconds the commands themselves took, not a jump). **Important nuance found:** for genuinely LIVE content, resuming rejoins the live broadcast edge rather than the paused instant — that's correct/expected (there's no "exact same moment" to return to on a live feed), and doesn't apply to DVR recordings, which is what Jeff actually watches most and where exact-position resume is what matters. **Why this is universal, not Fubo-specific:** `cmd media_session dispatch pause/play` operates through Android's system MediaSessionManager, not a per-app package call — Amazon's own Fire TV developer docs confirm MediaSession integration is required of all Fire OS apps precisely so the remote's physical play/pause button and Alexa voice control work uniformly. Same command, no per-app branching, should work the same way on Netflix/Prime/Max/Disney+/YouTube etc. as it did on Fubo. **Change applied:** `packages/hcc.yaml` line 367, `AI Show Camera on Fire TV` automation, step 1 — `"input keyevent 127"` → `"cmd media_session dispatch pause"` (camera view + 8s delay + HOME-return steps unchanged, already confirmed reliable). Edited via Studio Code Server's integrated terminal (sed, not the GUI editor — avoids the known Prettier format-on-save corruption risk), backed up the file first, validated with `ha core check` (passed), reloaded via Developer Tools → YAML → Automations (no restart needed) — automation shows enabled with zero errors. **Full end-to-end live verification:** fired a real simulated `codeproject_ai.object_detected` event (vehicle/driveway) while Jeff was actually watching live Fox News on a different input (confirmed via him he couldn't see this input, gave go-ahead to test) — confirmed via ADB screenshots + `dumpsys` that the sequence ran (camera view opened, HOME returned afterward, Fubo's process stayed alive — not killed), then reopened Fubo via a real D-pad tile select (not a raw relaunch intent, matching exactly how Jeff would do it by hand) — **it resumed to its own last in-app screen (the live Guide), not a cold splash restart**, confirming the app-state-preservation behavior survived the new pause step intact. Did not exhaustively re-test on Netflix/Prime/etc. live (would require actually playing something in each app) — confidence in universality rests on the command being Android/Fire-OS-level rather than per-app, per Amazon's own documentation, not on having clicked through every app.

- **07-15:** 🔧 **Two real follow-up fixes after Jeff used the Fire TV pop-up live: the ~5min delay, and the "restart the recording" annoyance.** (1) **Delay root-caused to Blink's default 5-minute cloud poll** (confirmed via HA's own Blink integration docs — not something today's or yesterday's automation work touched, it's been the baseline since Blink was set up 07-09). **Fix:** new automation **"Blink Fast Motion Poll"** in `packages/hcc.yaml` (`trigger: time_pattern seconds: "/30"` → `homeassistant.update_entity` on all 6 Blink motion `binary_sensor`s) forces a fresh poll every 30s instead of waiting on the default 5-min cycle. (2) **The Fire TV auto-return was making Jeff's actual problem worse, not better:** the 07-14 fix returned to a generic Home screen (fine but required a manual tap back into whatever app), so it was changed to relaunch FuboTV directly via `monkey -p com.fubo.firetv.screen -c android.intent.category.LAUNCHER 1` — but **live testing showed this was wrong**: Jeff is almost always watching either the Braves game live or a **DVR recording** on Fubo, and relaunching the app via its launcher intent makes Fubo **cold-start to its show-list/DVR menu** every time, discarding actual playback position — meaning Jeff had to restart the recording from scratch and fast-forward back to where he was. **Root cause understood, not just observed:** Android apps' launcher/dispatch activities commonly route ALL fresh launch-intent taps to a default/home screen by design, regardless of an existing backgrounded task — this is intentional app behavior, not something fixable from the HA/ADB side. Tried Android's standard "Recent Apps" task-switch (`input keyevent 187`, `KEYCODE_APP_SWITCH`) as a way to bring back the *existing* backgrounded task without re-triggering that cold-start routing — **doesn't work on this Fire OS build's launcher** (keycode had no visible effect, no recents UI appeared). **Reverted to `input keyevent 3` (HOME)** as the final, correct behavior — pressing Home never touches the backgrounded app's task/activity state at all (unlike a fresh launcher intent), so whatever Jeff was watching keeps playing/stays paused-in-place in the background; the only remaining ask on him is one manual tap back onto the Fubo tile, which then resumes exactly where he left off — no restart, no fast-forwarding. **If revisiting this later:** the real fix would be a way to bring an existing Android task to the foreground without invoking its launch-intent routing (e.g. `am` task-focus commands, if this Fire OS build ever supports them) — not found working here, HOME is the safe fallback. Both fixes validated via `ha core check` + `automation.reload` (no full restart needed), applied via the Terminal add-on per the established reliable method.

- **07-14 (cont.):** 🎉 **FIRE TV POP-UP — ACTUALLY FIXED, CONFIRMED WORKING LIVE (not via Alexa).** After confirming `alexa_media_player`'s synthetic command approach was a dead end (see previous 07-14 entry), found a genuinely different mechanism that works: instead of asking Alexa to "show" the camera, drive the Fire TV's own browser directly via **`androidtv.adb_command`** (already available — HA's `androidtv`/ADB integration backing `media_player.fire_tv_viewing_room` — no new integration needed). The camera entity's `entity_picture` (a signed `/api/camera_proxy/...?token=...` URL, refreshed live from HA's own state at automation run time) opened via `am start -a android.intent.action.VIEW -d "<url>"` renders the actual live snapshot fullscreen on the TV through the Fire TV's built-in Amazon Silk browser — proved this by screenshotting the Fire TV directly over ADB (not guessing) and seeing the real driveway photo on-screen, then again with Jeff physically watching the correct HDMI input confirm it. **Root cause of why Alexa never worked, for the record:** Amazon's official Fire TV Blink app is real but incompatible with this specific Fire TV Stick model (confirmed via the Amazon Appstore listing's own compatibility check, red ✕ next to "Jeffrey's Fire TV") — dead end, not pursued further (no sideloading). The Alexa `alexa_media_player` custom-command trick works for real spoken commands but Amazon's cloud doesn't honor the same command injected via the API for this device class — also a dead end. The ADB/browser route sidesteps both. **`AI Show Camera on Fire TV` in `packages/hcc.yaml` rewritten:** action is now (1) `androidtv.adb_command` opening `am start -a android.intent.action.VIEW -d "http://192.168.1.66:8123{{ state_attr('camera.' ~ camera_key, 'entity_picture') }}"` targeting `media_player.fire_tv_viewing_room`, (2) an 8s delay, (3) a second `androidtv.adb_command` sending `input keyevent 3` (HOME) to cleanly return to the Fire TV launcher afterward — confirmed via ADB that it lands on `com.amazon.tv.launcher` every time, not stuck in the browser (an earlier attempt using `input keyevent 4`/BACK only partially exited the browser, confirmed via ADB `mResumedActivity` still showing the Silk browser after one Back press — HOME is the reliable universal return). **Known UX tradeoff (by design, not a bug):** this takes over the whole screen like switching apps, not a small toast overlay layered over what's playing (that would've needed the original Kodi plan) — acceptable tradeoff since it actually works and Kodi was explicitly declined. **Verified end-to-end live, twice, through the real automation** (motion sensor forced via HA's own API → real `codeproject_ai` detection of 2 real cars in the driveway → both phone push and Fire TV camera pop-up fired) — not a simulated/faked event. `packages/hcc.yaml` edited via the Terminal add-on (`sed`, the established reliable method — Studio Code Server's Monaco editor had a focus-stealing UI glitch mid-session that nearly caused a bad edit, caught via `hass.callApi('POST','config/core/check_config')` before it reached disk; no actual corruption occurred, disk state was verified valid throughout). `ha core check` clean, `automation.reload` applied (no full restart needed).

- **07-14 (cont.):** 🔬 **Fire TV pop-up tested live twice, real root cause narrowed — it's the synthetic command delivery, not Blink/Alexa linking.** After the CodeProject.AI fix above, forced `binary_sensor.301_driveway_motion` to `on` via `hass.callApi` (not a fake event — the real automation chain, verified via HA's own trace: `image_processing.codeproject_ai_object_301_driveway` genuinely detected `{"car": 2}` both times) — **twice**. Both times "AI Object Detected Notify" and "AI Show Camera on Fire TV" fired correctly (confirmed via automation `last_triggered` timestamps and the trace showing the exact `media_player.play_media` call with `media_content_id: "show me the 301 driveway camera"` dispatched to `media_player.jeffrey_s_fire_tv`, no error). **Both times: phone push arrived (confirmed by Jeff), TV pop-up did not appear** (Jeff was on the correct HDMI input the second time). Ruled out: cameras not linked to Alexa (Jeff confirmed in the Alexa app they ARE linked — checked directly, not assumed) and wrong phrase (Jeff spoke the exact same phrase, **"Alexa, show me the 301 driveway camera," out loud and it correctly pulled up the camera on the Fire TV** — so the camera name, the phrase, and Alexa's routing all work for a real voice command). **Conclusion: `alexa_media_player`'s synthetic `media_player.play_media` / `media_content_type: custom` trick is not equivalent to a real spoken command for this camera-show use case** — a real voice command lets Alexa's cloud intelligently pick/cast to a screen, but injecting the command directly at the Fire TV's own media_player entity apparently doesn't trigger the same cross-device display behavior. This is a known-rough-edge class of issue with that HACS integration's custom-command trick, not a Blink/HA/camera problem. **Not fixed yet — deliberately not guessing at a live YAML change without being able to verify it** (that's exactly the "declared done without testing" pattern this whole audit was trying to undo). **Next step for whoever picks this up:** research `alexa_media_player`'s GitHub issues for the correct way to trigger a camera "Show" on a specific screen via its service calls (may need to target a different entity than the Fire TV's own, or use a documented `extra`/`queue_delay` param, or accept this needs a real Echo device as the command source with the Fire TV only as the display target). (Tried to independently verify via screenshotting the Alexa Windows app myself first — hit a real dead end: this session's terminal runs elevated/Administrator, and Windows blocks synthetic mouse/keyboard input from an elevated process into a non-elevated app window (UIPI) — window focus-switching works, clicking does not. Worth remembering for any future desktop-app automation on the beast: don't run the terminal elevated if GUI-clicking a normal app is needed.)
