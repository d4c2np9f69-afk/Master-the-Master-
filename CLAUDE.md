...
9. **📺 Fire TV motion pop-up alerts — ✅✅ ACTUALLY FIXED (07-14, later same day) — see the second 07-14 changelog entry for the full story.** The Alexa-based approach below (and its 07-14 "not working" correction) is now superseded: the working mechanism is `androidtv.adb_command` opening the camera's live snapshot URL directly in the Fire TV's browser, not Alexa at all. Verified live twice through the real automation. If touching this automation again, the current action lives in `packages/hcc.yaml` under `alias: "AI Show Camera on Fire TV"` — do not revert to the `media_player.play_media`/Alexa custom-command approach, it's a confirmed dead end for this device.
