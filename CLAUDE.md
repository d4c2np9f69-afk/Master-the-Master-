...
9. **📺 Fire TV motion pop-up alerts — MECHANISM PROVEN WORKING (07-11), YAML NOT YET WRITTEN INTO `packages/hcc.yaml`.** Fire TV paired to HA (`media_player.fire_tv_viewing_room` via Android Debug Bridge) and separately connected via **`alexa_media_player`** HACS integration (Jeff completed the Amazon login himself — Claude never handles credentials) → real entity confirmed: **`media_player.jeffrey_s_fire_tv`**. **Live-tested and confirmed by Jeff on the actual TV screen:** calling `media_player.play_media` with `media_content_type: custom` and `media_content_id: "show me the driveway camera"` targeted at `media_player.jeffrey_s_fire_tv` makes the Fire TV pop up that camera's live view immediately, exactly like speaking the command to it. **Exact YAML to add** (not yet applied — see blocker below):
   ```yaml
   - service: media_player.play_media
     target:
       entity_id: media_player.jeffrey_s_fire_tv
     data:
       media_content_id: "show me the {{ camera_friendly_name }} camera"
       media_content_type: custom
   ```
   Add this as a new action inside the existing `AI Object Detected Notify` automation (reuses its person/vehicle/animal classification + per-camera mute gate — same as the phone-notification path). Also still need a **new arrival-suppression automation**: trigger on `person.*` (Angela/Jeff/Braxton) going `not_home`→`home`, action = set the matching `input_datetime.hcc_ai_mute_<camera>` helper ~10 min out, so cameras near the door don't alert on family walking in.
   **Blocker: the legacy "File editor" HA add-on (`core_configurator`) is too fragile to safely hand-edit `packages/hcc.yaml` via browser automation** — its code editor doesn't recognize special keys properly (a `Page_Up` keypress got typed as the literal string "Page_Up" into line 153 during this session; caught immediately and undone with Ctrl+Z before saving, so the file is untouched, but it's a real corruption risk). `AI Object Detected Notify` also **cannot be edited via HA's own Automations UI** — it's package-defined, not in `automations.yaml`, so the UI only offers a "Migrate" button (do NOT click this without asking Jeff — it would move the automation out of the organized `packages/hcc.yaml` structure). **Recommended fix for next session:** install the **Studio Code Server** HA add-on (modern VS Code-based editor, much more reliable than the legacy File Editor) before attempting this edit again, or have Jeff paste the YAML above himself.
