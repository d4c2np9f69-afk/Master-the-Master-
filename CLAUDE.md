...
0. **▶️ PICK UP HERE (07-03 EOD).** Tonight landed big: Beehive **online** in the app, water+gas meters **live & correct**, real KTNWHITE21 weather **live in HA** (`sensor.backyard_temperature/_feels_like/_humidity/_wind`), and the **Alexa↔HA skill is already linked** (confirmed in the Alexa app). **Next, right where we stopped:** (a) HA → **Settings → Voice assistants → Expose** → expose `Backyard Temperature` + `Backyard Humidity` (+ others) to **Amazon Alexa** → "Alexa, discover devices" → test "**Alexa, what's the backyard temperature?**" (if the wording trips her, set the Alexa alias to just "Backyard"). (b) Then build the **utility helper tiles** (This-Month/Flow/Cost) per `docs/beehive/ha_helpers_and_alexa.md` — sources are `sensor.water_meter_reading` (÷10) and `sensor.gas_meter_reading` (÷100). (c) Optional: retry **Blink** via the HA wizard (patched `hcc-2fa-fix` is installed but login still failing — likely still the upstream 2FA bug). Blink/mPING/Mend are NOT priorities.
