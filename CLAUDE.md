...
0. **▶️ PICK UP HERE (updated 07-20).** **Top priority: confirm `loewenhome.com` is serving the new `no-cache` header on `service-worker.js`** (see 07-20 changelog — `_headers` fix pushed and verified live on `toro1-5rz.pages.dev`, but `loewenhome.com` custom domain was still returning the stale 4h-cache header as of this writing; coworker has no Cloudflare dashboard/API access to purge/investigate zone Cache Rules, this session does). `curl -sI https://loewenhome.com/service-worker.js` should show `Cache-Control: no-cache` — if not, purge cache for that path in the Cloudflare dashboard and check for a zone-level Cache Rule overriding origin headers. **Other remaining next steps:** (a) build **utility helper tiles** (This-Month/Flow/Cost) per `docs/beehive/ha_helpers_and_alexa.md`. (b) Fix `HCC — Freeze Warning` automation → repoint to `sensor.backyard_temperature`. (c) **⛽ GAS billing sync — WAITING on Jeff's first Spire bill.** Water is fully bill-synced ($72.42 validated). (d) **iPad Air 2 wall-display setup** — AbortSignal.timeout Safari-15 polyfill deployed + working, but HA token persistence + "Add to Home Screen" + Guided Access still need final confirmation (see 07-15 changelog).
