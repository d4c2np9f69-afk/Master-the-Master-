...
- **07-02:** 🎉 **J45 migrated to internal drive** — Beehive now boots HA OS 18.1 standalone off the internal 128 GB SSD (was flaky external USB). Full backup → Ubuntu 26.04 live → flash to internal → restore. USB ports freed for RTL-SDR/Zigbee. Prepped **`docs/beehive/rtl_sdr_meter_setup.md`** (rtlamr2mqtt path for the Itron ERT water+gas meters — **no Windows drivers, add-on ships the Linux driver**). Still need the full **gas ERT ID** (`…333930…`).
