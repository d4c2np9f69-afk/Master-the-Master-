...
16. **"Alexa fast-forward" broken — root cause narrowed to two candidates, needs local/ADB + Alexa-app access this cloud session doesn't have (08-03).** Found the real feature: `script.hcc_skip_commercial` in `beehive-config/hcc.yaml` (3× ADB `keyevent 90` + resume via `keyevent 126`, confirmed correct keycodes via research). **One specific ask for Jeff/coworker, in order:** (1) Developer Tools → Actions → run `script.turn_on` on `script.hcc_skip_commercial` directly — if the TV skips forward, the script and ADB link are both fine and the fix is purely re-linking the Alexa Routine/expose in the Alexa app (not an HA problem); (2) if nothing happens, check `media_player.fire_tv_viewing_room` in Developer Tools → States for `unavailable` — ADB pairing has likely dropped (known to happen silently after a Fire TV firmware update/reboot) and needs re-pairing (Settings → Devices & Services → the Android TV/ADB integration → reconfigure, or re-enable ADB debugging on the Fire TV under Settings → My Fire TV → Developer Options). Also worth asking whether the existing `HCC Watchdog — Integration Down Alert` automation (already watches `media_player.fire_tv_viewing_room` for 5+ min `unavailable`) has fired recently — if not, the ADB link probably hasn't dropped and the break is upstream in Alexa. This single check would also explain the PiPup camera-popup automation failing, since both depend on the same ADB link — worth checking alongside item 12's still-open Fire TV PiP delay investigation. Separately: confirm with Jeff whether he means the `script.hcc_skip_commercial` Routine phrase or Amazon's own native built-in Fire TV voice fast-forward — the latter is outside this project entirely, no HA/app fix applies.
