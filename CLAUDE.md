...
- **07-04:** 💡 **Lighting control card added to HOME GUARDIAN** (Jeff's "add the auto lighting I have now"). `loadLights()` pulls all HA `light.*` from `/api/states`; card shows **ALL ON / ALL OFF**, **100% / 50% / 25%** brightness presets (mirrors his RF remote), and a per-bulb ON/OFF pill list. Controls via `light/turn_on`|`turn_off` service calls (with `brightness_pct`) through the `/api/ha` proxy — same pattern as Away Mode. Loads on the Guardian tab + in the 60s self-heal loop. Honest empty-state hint when no lights are in Beehive yet. Verified in harness: renders, toggle/all/brightness fire the correct service payloads, zero JS errors. **Jeff's hardware:** (1) **SYLVANIA Smart WiFi bulbs** = Tuya-based (SYLVANIA app is a rebranded Smart Life/Tuya app) → add to Beehive via the **Tuya** integration (Smart Life QR login) or **LocalTuya**; then they auto-appear in this card. (2) **RF Wireless LED strip remote** (ON/OFF/100/50/25/MODE/SPEED) = dumb 433MHz, NOT HA-controllable directly → needs a **Broadlink RM4** to learn the codes, OR (better) swap the controller for a cheap WiFi (Magic Home/Tuya) or **WLED-on-ESP** controller → becomes a native `light.*` with full control. Doc pending if Jeff wants click-by-click.
...
4. **🎥 Blink cameras — JEFF'S #1 WANTED FEATURE (TOP PRIORITY, not optional). Upstream HA 2FA bug; we built our own fix — pending live test.** The HA Blink 2FA flow is broken for everyone: Blink's server returns `error_cause=empty_cookies` because HA's **shared** aiohttp session drops the auth cookies between the login step and the 2FA PIN step (blinkpy 0.25.2 / HA 2026.6.x; no official fix as of 07-03 — verified via blinkpy **#1217**, core **#173419/#173520/#168029**). **OUR FIX (07-03, `beehive/blink/config_flow.py`):** give the config flow ONE dedicated client session (own cookie jar), reused across login + 2FA, so cookies survive to the PIN (this mirrors why standalone blinkpy login works). **Install:** HA Terminal → `cd /config/custom_components/blink && curl -fsSL "https://toro1-5rz.pages.dev/beehive/blink/config_flow.py" -o config_flow.py && curl -fsSL "https://toro1-5rz.pages.dev/beehive/blink/manifest.json" -o manifest.json` → restart HA → **delete** the Blink integration → **re-add** (email/pw → SMS PIN). Then cameras flow to the app (HOME → Cameras) via `loadCameras()`. **⚠️ CAN'T be verified from dev side (needs live Blink account + Amazon auth) — only Jeff's install confirms.** **If it still fails:** fallback = generate the Blink token with a standalone blinkpy script (single session keeps cookies) and import it; also keep checking for an official upstream fix each session. **Do NOT deprioritize cameras.**
